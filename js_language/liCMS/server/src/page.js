...
function appRoute(app, db){
    app.post('/api/v1/page/create', (req, res) => {
        let promise = crud.createX(req, res, db, db.pages, {method:'token'});
        promise.then(
            function(result){
    ...
    app.get('/api/v1/page/list', (req, res) => {
        let promise = crud.listX(req, res, db, db.pages, {method:'token'});
        promise.then(
            function(result){
                res.setHeader('Content-Type', 'application/json');
                res.send(JSON.stringify(result));
    ...
    app.patch('/api/v1/page/:xId', (req, res) => {
        let promise = crud.updateX(req, res, db, db.pages, {method:'token'});
        promise.then(
            function(result){
                res.setHeader('Content-Type', 'application/json');
                res.send(JSON.stringify(result));
    ...
    app.delete('/api/v1/page/:xId', (req, res) => {
        let promise = crud.delX(req, res, db, db.pages, {method:'token'});
        promise.then(
            function(result){
                res.setHeader('Content-Type', 'application/json');
                res.send(JSON.stringify(result));
